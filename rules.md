**CODE WAR** 

* Open to registered students; individual or teams of 2
* Problems: easy, medium, hard within time limit
* Languages: C, Java, Python
* No late submissions
* No AI/tools/external help
* Use assigned systems only
* Tie: faster submission wins
* Judges’ decision final; results after evaluation

---

**TECH QUIZ** 

* Teams of 2 only
* Round 1: 12 MCQs, 15 mins, 12 points
* Round 2: Unscramble (buzzer), 8 points
* Round 3: Identify logo/image, 2 mins, 6 points
* No phones in Round 1
* Limited phone use for buzzer in Rounds 2 & 3
* No communication between teams
* Be present 10 mins early

---

**IDEATHON X** 

* Team of 2–3 (inter-department allowed)
* ID card and at least one laptop required
* Domains: AI, Web, IoT, Cybersecurity
* Must build idea during event
* 7–10 min presentation
* Judging: innovation, feasibility, relevance, presentation
* No pre-built projects or cheating

---

**PHOTOGRAPHY & VIDEOGRAPHY** 

* Open to registered participants (1st–3rd year only)
* Conducted offline
* Content must be captured on campus
* Follow theme and originality
* No old/downloaded content
* Only basic editing allowed
* No heavy editing/manipulation

---

**RAPID TYPING** 

* Time: 2 min 30 sec
* Type given passage exactly
* Accuracy + speed important
* No auto-correct, copy-paste, or help
* Score: WPM (gross & net)
* Start/stop as instructed

---

**POSTER PRESENTATION** 

* Team of 3 members
* 5–7 min presentation + 2–3 min Q&A
* Original poster required
* Size: A2 or A3
* Must match technical theme
* Judging: creativity, content, clarity
* Late arrival may disqualify

---

**TECHNICAL DEBATE** 

* Individual participation
* Conducted in English
* Assigned “For” or “Against”
* 2–3 min opening, 1–2 min rebuttal, 1 min conclusion
* No gadgets/internet
* Judging: knowledge, clarity, arguments, time
* No misconduct or irrelevant content

---

**THE NEXTGEN GAMING** 

* BGMI & Free Fire squad matches
* Minimum 3 players per squad
* Multiple rounds + final
* Map decided by organizers
* Fair play mandatory
* No cheating/hacking
* Report on time

---

**PROTOFORGE AND CONSTRUCTION** 
*Protoforge:*

* Individual CAD-based 3D modeling
* Complete within time limit
* Judges’ decision final

*Construction:*

* Team of 2–4
* Build bridge in 30 mins + 15 mins drying
* Load applied until failure
* Materials provided; glue to be brought
* Rules must be followed strictly

---

**TREASURE HUNT** 

* Individual/team participation
* Solve clues in order
* No tampering with clues
* No cheating or outside help
* Follow safety rules
* No clue sharing
* Winner based on time/accuracy

---

**THE CIRCUIT ARENA** 

* Team of max 2
* Round 1: MCQ elimination
* Round 2: Circuit building (time-based)
* Round 3: Debugging faulty circuit
* Knowledge of analog electronics required
* Judges’ decision final
